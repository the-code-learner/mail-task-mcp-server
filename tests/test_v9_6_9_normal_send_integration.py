...
class NormalSendIntegrationV969Tests(unittest.TestCase):
    ...
    def test_real_normal_send_to_bcc_has_private_bcc_and_one_canonical_sent(self):
        ...
        self.assertEqual(
            {(row["recipient"], row["recipient_role"]) for row in result["deliveries"]},
    ...
    def test_real_normal_send_to_cc_bcc_preserves_visible_headers_and_private_bcc(self):
        result = self._send(
            to=["a@example.com"],
            cc=["b@example.com"],
            bcc=["c@example.com"],
        ...
        self.assertEqual(
            {(row["recipient"], row["role"]) for row in saved["deliveries"]},
            {
                ("a@example.com", "to"),
                ("b@example.com", "cc"),
                ("c@example.com", "bcc"),
