...
from postmaster.mcp_v969 import MCP_COMMAND_COUNT_V969, install_runtime_v969_mcp
...
class McpV969Tests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.env = patch.dict(os.environ, {"POSTMASTER_RUNTIME_CONTROL_PATH": str(self.control)}, clear=False)
        self.env.start()
        ...
        self.releases = patch.object(
            runtime_control, "stable_release_tags",
    ...
    async def test_command_count_delta_execute_schema_and_explicit_remote_fetch(self):
        async with Client(self.core.mcp) as client:
            ...
            execute_props = by_name["runtime_version_change_execute"].input_schema["properties"]
            self.assertIn("preview_id", execute_props)
            self.assertNotIn("confirmation_token", execute_props)
            proxy_execute_props = by_name["privacy_proxy_provisioning_execute"].input_schema["properties"]
            self.assertIn("preview_id", proxy_execute_props)
            self.assertNotIn("confirmation_token", proxy_execute_props)

            safe = _payload(await client.call_tool(
            ...
            fetched = _payload(await client.call_tool(
                "fetch_email_remote_content",
                {
                    "mailbox": "INBOX", "uid": "7", "account_id": "acct",
    ...
    async def test_runtime_preview_execute_without_bearer_replay_and_stale_rejection(self):
        with patch.object(runtime_control, "schedule_current_process_termination") as restart:
            async with Client(self.core.mcp) as client:
                preview = _payload(await client.call_tool(
                    "runtime_version_change_preview",
                ...
                executed = _payload(await client.call_tool(
                    "runtime_version_change_execute",
                    {"operation": "pin-version", "target_version": "v9.6.7"},
                ))
                replay = _payload(await client.call_tool(
                    "runtime_version_change_execute",
                    {"operation": "pin-version", "target_version": "v9.6.7"},
                ))

                stale_preview = _payload(await client.call_tool(
                    "runtime_version_change_preview",
                    {"operation": "pin-version", "target_version": "v9.6.7"},
                ))
                self.state["build"] = "v9.6.8-mutated"
                stale = _payload(await client.call_tool(
                    "runtime_version_change_execute",
                    {
                        "operation": "pin-version", "target_version": "v9.6.7",
