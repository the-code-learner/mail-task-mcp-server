...
- normal source tree under `src/nomadcompass/` instead of Python embedded inside Compose `configs:`;
...
```yaml
NOMADCOMPASS_REPO: the-code-learner/mail-task-mcp-server
NOMADCOMPASS_REF: v9-structural-runtime
```

A mutable branch is convenient while testing. For production, pin `NOMADCOMPASS_REF` to an immutable release tag or commit SHA.
...
```text
context-model-v1
nomadcompass-context-mrl-128d-int8.tar.gz
```
...
```yaml
NOMADCOMPASS_REF: v9-structural-runtime
NOMADCOMPASS_REFRESH_ON_START: 'true'
```
