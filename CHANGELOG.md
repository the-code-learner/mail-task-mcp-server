...
- Extended logical outbound persistence for normal group sends without changing transport semantics: one MIME/DATA submission can have multiple SMTP RCPT recipients while To/Cc/Bcc roles are mapped privately to the same `out_*` operation and one canonical Sent. Inbound `In-Reply-To`/`References` correlation can resolve any persisted delivery Message-ID, including the second fanout delivery, back to the existing logical root without requiring the canonical Sent Message-ID to match or creating a second outbound root.
...
- Added `/data/mcp_pending_approvals_v969.db` for pending approval state and `/data/outbound_operations_v969.db` for sender-private logical-operation/Sent/Bcc delivery mapping. The existing `/data/mailbox_cache.db` remote-resource table is reused for top-level and nested passive positive/negative cache entries; no separate CSS store is introduced and newly generated cache keys are opaque hashes.
